    ...
    <div class="page-card list-card">
      <div class="page-head">
        <div>
          ...
          <p>
            查看策略回测结果、交易表现与处理动作，支持新建、比较和批量删除。
          </p>
        </div>
        <div class="actions">
          <el-button
            type="primary"
            @click="router.push('/barginhunting/strategy/strategyadd')"
          >
            <el-icon><Plus /></el-icon>
            <span>新建策略</span>
          </el-button>
          <el-button
            v-if="selectedId.length === 2"
            class="btn-violet"
...
async function getList() {
  ...
  try {
    ...
    list.value = (data.strategyList || []).map((item) => ({
      ...item,
      average:
        item.balance && item.tradeNum
          ? (item.balance / item.tradeNum).toFixed(2)
          : item.average
    }))
  } finally {
...
async function confirmDel(row, index) {
  await ElMessageBox.confirm('此操作将删除此策略，是否继续？', '提示', {
    type: 'warning'
  })
...
async function confirmDelSelected() {
  ...
  await ElMessageBox.confirm(
    `此操作将删除${selectedId.value.length}个策略，是否继续？`,
    '提示',
    { type: 'warning' }
  )
  ...
  list.value = list.value.filter(
    (item) => !selectedId.value.includes(item.strategyId)
  )
  selectedId.value = []
...
function compareSelected() {
  ...
  router.push(
    `/barginhunting/analyte/verification/compare/${selectedId.value[0]}/${selectedId.value[1]}`
  )
}
</script>
