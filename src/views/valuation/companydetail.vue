  ...
  <section v-loading="loading" class="detail-shell">
    ...
    <div class="detail-card">
      ...
      <el-descriptions :column="4" border>
        ...
        <el-descriptions-item label="大V推荐得分">{{
          valuation.recommendationScore
        }}</el-descriptions-item>
        <el-descriptions-item label="财务评价得分">{{
          valuation.financialScore
        }}</el-descriptions-item>
        <el-descriptions-item label="估值">{{
          valuation.valuation
        }}</el-descriptions-item>
        <el-descriptions-item label="预估/假定增长率">
          {{ valuation.growthRatePrediction }} /
          {{ valuation.growthRateAssumption }}
        </el-descriptions-item>
        ...
        <el-descriptions-item label="PE-TTM">{{
          safeRound(valuation.pe)
        }}</el-descriptions-item>
        <el-descriptions-item label="PB(不含商誉)">{{
          safeRound(valuation.pb)
        }}</el-descriptions-item>
        <el-descriptions-item label="市值">{{
          formatYi(valuation.marketValue)
        }}</el-descriptions-item>
        <el-descriptions-item label="股本">{{
          formatYi(valuation.capitalization)
        }}</el-descriptions-item>
        <el-descriptions-item label="分红率">{{
          formatPercent(valuation.dividendRate)
        }}</el-descriptions-item>
        <el-descriptions-item label="连续分红年数">{{
          valuation.dividendYears
        }}</el-descriptions-item>
        <el-descriptions-item label="上市以来持续分红">{{
          valuation.dividendIsSure
        }}</el-descriptions-item>
        <el-descriptions-item label="股息率">{{
          formatPercent(valuation.yield)
        }}</el-descriptions-item>
        <el-descriptions-item label="五年平均股息率">{{
          formatPercent(valuation.yieldAverage)
        }}</el-descriptions-item>
        <el-descriptions-item label="前瞻股息率">{{
          formatPercent(valuation.yieldPrediction)
        }}</el-descriptions-item>
      </el-descriptions>
    </div>
    ...
    <div class="detail-card">
      ...
      <el-descriptions :column="4" border>
        ...
        <el-descriptions-item label="净资产折价率">{{
          valuationData.netAssetDiscount
        }}</el-descriptions-item>
        <el-descriptions-item label="净资产估值">{{
          safeRound(valuationData.netAssetValuation)
        }}</el-descriptions-item>
        <el-descriptions-item label="分红稳定性">{{
          formatPercent(valuationData.dividendStability)
        }}</el-descriptions-item>
        <el-descriptions-item label="预期分红率">{{
          formatPercent(valuationData.dividendRatePrediction)
        }}</el-descriptions-item>
        <el-descriptions-item label="预期营收增速">{{
          formatPercent(valuationData.incomeGrowthRatePrediction)
        }}</el-descriptions-item>
        <el-descriptions-item label="预期利润增速">{{
          formatPercent(valuationData.profitGrowthRatePrediction)
        }}</el-descriptions-item>
        <el-descriptions-item label="次期增速推算">{{
          formatPercent(valuationData.growthRatePrediction)
        }}</el-descriptions-item>
        ...
        <el-descriptions-item label="高增长折现系数累积">{{
          valuationData.highGrowthDiscountCoefficientSum
        }}</el-descriptions-item>
        ...
        <el-descriptions-item label="永续期折现系数累积">{{
          valuationData.perpetualDiscountCoefficientSum
        }}</el-descriptions-item>
        <el-descriptions-item label="永续期估值">{{
          valuationData.perpetualValuation
        }}</el-descriptions-item>
        <el-descriptions-item label="市场风险">{{
          valuationData.marketRisk
        }}</el-descriptions-item>
        <el-descriptions-item label="行业风险">{{
          valuationData.industryRisk
        }}</el-descriptions-item>
