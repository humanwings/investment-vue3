    ...
    <div class="page-card list-card">
      ...
      <div class="table-shell table-shell--fill">
        ...
        <el-table-column label="关注度" width="120">
          <template #default="{ row }">
            <el-rate :model-value="Number(row.popularity) || 0" :max="3" disabled />
          </template>
        </el-table-column>
        ...
        <el-table-column label="暴跌总幅度" width="130">
          <template #default="{ row }">{{ formatPercent(row.plungeSum) }}</template>
        </el-table-column>
        <el-table-column label="N-1日跌幅" width="110">
          <template #default="{ row }">{{ formatPercent(row.valleyRange1) }}</template>
        </el-table-column>
        <el-table-column label="N日跌幅" width="110">
          <template #default="{ row }">{{ formatPercent(row.valleyRange2) }}</template>
        </el-table-column>
        <el-table-column fixed="right" label="处理" width="220">
          <template #default="{ row, $index }">
            <div class="row-actions">
              <el-button v-if="row.ndate" text type="primary" @click="showBuy(row)">
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="520px">
      <el-form ref="formRef" :model="temp" :rules="rules" label-width="100px">
        ...
        <el-form-item label="Date" prop="ndate">
          <el-date-picker v-model="temp.ndate" type="date" value-format="YYYY-MM-DD" />
        </el-form-item>
...
import { createAnalyte, deleteAnalyte, getWaitList, updateAnalyte } from '@/api/analyte'
